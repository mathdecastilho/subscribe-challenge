# Handles the basic sales tax rule in the chain of responsibility.
#
# Basic sales tax is 10% on all items except books, food, and medical products.
...
# so callers only need to invoke the first link in the chain.
class BasicTax
  RATE_PERCENT              = 10
  ...
  # Returns the total tax (basic + downstream handlers) for one unit of +item+,
  # expressed in integer cents.
  #
  # @param item [Item]
